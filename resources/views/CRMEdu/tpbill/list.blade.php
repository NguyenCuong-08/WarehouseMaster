...
if (@$_GET['saler_id'] != null) {
    ...
    $members = \App\Models\Admin::where('room_id', \Auth::guard('admin')->user()->room_id)->pluck('id')->toArray();

    $whereSale = 'saler_id IN ('.implode(',', $members).')';
}
...
?>
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
                ...
                <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--scroll kt-datatable--loaded"
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach($listItem as $item)
                            <?php
                                if(is_object($item->bill_finance)) $da_thu += @$item->bill_finance->received;
                                if(is_object($item->bill_finance)) $chua_thu += @$item->bill_finance->total - @$item->bill_finance->received;
                            ?>
        ...
        <div class="lead-chart">
            ...
            $doanh_so = \App\CRMEdu\Models\Bill::whereRaw($whereSale)
                        ->whereRaw($whereRegistration_date)
                        ->sum('total_price');

            ?>
            <span>Doanh số: {{ number_format($doanh_so, 0, '.', '.') }}đ.</span>&nbsp;&nbsp;
            <span>Đã thu trong trang này: {{ number_format($da_thu, 0, '.', '.') }}đ.</span>&nbsp;&nbsp;
            <span>Chưa thu trong trang này: {{ number_format($chua_thu, 0, '.', '.') }}đ.</span>&nbsp;&nbsp;
        </div>
    </div>
@endsection
        ...
        $(document).ready(function () {
            $('table td').hover(function () {
                var div = $(this).find('.div-tooltip_info');
                    ...
                    var id = div.parents('td').find('td:first-child').text();
                    $.ajax({
