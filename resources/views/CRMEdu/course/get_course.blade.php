...
@section('main')
    ...
    <div class="col-xs-12 menu-document">
        <ul class="nav">
            ...
            <?php
            $categories = \App\CRMEdu\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1);
            ...
            @foreach($categories as $cat)
                    <?php
                    $cat_childs = \App\CRMEdu\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1)
                    ...
                    @if(count($cat_childs) > 0)
                        <ul class="dropdown-menu">
                            @foreach($cat_childs as $cat_child)
                                    <?php
                                    $cat3_childs = \App\CRMEdu\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1)
                            ...
                            <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
                                ...
                                @foreach($module['list'] as $field)
                                    <td data-field="{{ @$field['name'] }}"
                                        ...
                                        @else
                                            @include('CRMEdu.course.list.td.view_course')
                                        @endif
                                    </td>
                                @endforeach
                            </tr>
