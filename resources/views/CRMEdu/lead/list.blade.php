...
// dd($whereCreated_at);
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        $count_khqt_cao = \App\CRMEdu\Models\Lead::whereRaw($whereLikeSale)
            ...
            // ->where('created_at', '<=', date('Y-'.date('m', strtotime($day)).'-t 23:59:59', strtotime($day)))
            ->count();
        $count_co_hoi = \App\CRMEdu\Models\Lead::whereRaw($whereLikeSale)
        ...
        $plan = \App\CRMEdu\Models\Plan::whereRaw($whereSale)->orderBy('id', 'desc')->first();
        $ds_tuan = \App\CRMEdu\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-' . date('w') . ' days')))
            ->sum('total_price');
        $ds_thang = \App\CRMEdu\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-01 00:00:00', time()))
            ->sum('total_price');
        $hd = \App\CRMEdu\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-' . date('w') . ' days')))
            ->count();
        ...
        <div class="lead-chart">
            ...
            <span><a target="_blank"
                     href="/admin/timekeeping/add">Tương tác: {{ $coun_lead_contacted_logs }}/60.</a></span>&nbsp;&nbsp;&nbsp;&nbsp;
            <span>KHQT mới: {{ $quanTamMoi }}/{{ @$plan->khqt_moi }}.</span>&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <div class="loc-rate" style="margin-bottom: 5px;">
            <?php
            $rates = CommonHelper::getFromCache('tags_lead_rate', ['tags']);
            if (!$rates) {
                $rates = \App\Models\Tag::select(['id', 'name', 'color'])->where('type', 'lead_rate')->where('status', 1)->orderBy('order_no', 'desc')->get();
                CommonHelper::putToCache('tags_lead_rate', $rates, ['tags']);
            }

            ?>
            @foreach ($rates as $rate)
                <a class="kt-badge kt-badge--default kt-badge--inline kt-badge--pill"
                   href="/admin/lead?search=true&rate%5B%5D={{ $rate->id }}" style="cursor:pointer;
                border: 1px dashed #ccc; color: {{ $rate->color }}; {{ is_array(@$_GET['rate']) && in_array($rate->id, @$_GET['rate']) ? 'border: 1px inset #5867dd;' : '' }}">{{ $rate->name }}</a>
            @endforeach
                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        @if(in_array('lead_float_view', $permissions))
                            <div>
                                <a href="/admin/lead"
                                   class="btn {{ explode('?', $_SERVER['REQUEST_URI'])[0] == '/admin/lead' ? 'btn-primary' : 'btn-default' }}">Khách
                                    của tôi</a>
                                <a href="/admin/lead/chua-chia"
                                   class="btn {{ strpos($_SERVER['REQUEST_URI'], '/chua-chia') != false ? 'btn-primary' : 'btn-default' }}">Khách
                                    chưa chia</a>
    ...
    @if(in_array($module['code'] . '_assign', $permissions))
        <!-- Nếu có quyền chuyển lead thì thêm phần chuyển lead -->
        @include('CRMEdu.lead.list.partials.lead_assign')
    @endif

    @if(in_array(CommonHelper::getRoleName(\Auth::guard('admin')->user()->id, 'name'), ['telesale', 'sale', 'truong_phong_sale']))
        @include('CRMEdu.lead.list.partials.nhac_nho_telesale')
    @endif
@endsection
                ...
                if (div.html().trim() == '<img class="tooltip_info_loading" src="/images_core/icons/loading.gif">') {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            // console.log('tooltip-info', result)
                            div.html(result);
                        },
                        error: function () {
