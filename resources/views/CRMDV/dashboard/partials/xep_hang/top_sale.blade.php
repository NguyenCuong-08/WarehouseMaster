                ...
                <table class="table table-striped">
                    ...
                    @foreach($best_sales as $v)
                        ...
                        <tr>
                            ...
                            <td @if(date('d')/30 > $v->total_price/10000000) class="text-red" @endif>{{ number_format($v->total_price, 0, '.', '.') }}<sup>đ</sup></td>
                        </tr>
                    @endforeach
                    <?php
                    krsort($ds_phong);
                    ...
                    <tr>
                        ...
                        <td><strong><a target="_blank" href="/admin/bill?search=true&from_date={{ date('Y-m-01') }}&registration_date=1"
                                    style="display: inline-block;">{{ number_format($tong_doanh_so, 0, '.', '.') }}<sup>đ</sup></a></strong></td>
                    </tr>
                    </tbody>
                </table>
                @foreach($ds_phong as $room_id => $ds)
                    {{ $room_ids[$room_id] }}: {{ number_format($ds, 0, '.', '.') }}<sup>đ</sup><br>
                @endforeach
