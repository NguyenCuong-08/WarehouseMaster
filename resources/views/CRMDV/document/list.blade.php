...
@section('main')
    ...
    <div class="col-xs-12 menu-document">
        <ul class="nav">
            <li class="nav-item">
                <a class="nav-link {{ 0 == $category_id ? 'active' : '' }}" href="/admin/document/view/0">
                    Tất cả
                </a>
            </li>

            <?php 
                $categories = \App\CRMDV\Models\Category::select('id', 'name')->where('status', 1)->where('type', 5)
            ...
            @foreach($categories as $cat)
                <?php 
                    $cat_childs = \App\CRMDV\Models\Category::select('id', 'name')->where('status', 1)->where('type', 5)
            ...
            <li class="nav-item">
                <a class="nav-link {{ $cat->id == $category_id ? 'active' : '' }}" href="/admin/document/view/{{ $cat->id }}">
                    {{ $cat->name }}
                </a>
                ...
                <ul class="dropdown-menu">
                    @foreach($cat_childs as $cat_child)
                        <?php 
                            $cat3_childs = \App\CRMDV\Models\Category::select('id', 'name')->where('status', 1)->where('type', 5)
                                                ->where('parent_id', $cat_child->id)
                                                ->orderBy('order_no', 'desc')->orderBy('name', 'asc')
                                                ->get();
                        ?>
                        <li class="nav-item">
                            <a class="dropdown-item" href="/admin/document/view/{{ $cat_child->id }}">{{ $cat_child->name }}</a>
            ...
            <li class="nav-item">
                <a class="nav-link " href="/admin/category_document" style="margin-right: 20px;
    color: #fff;
