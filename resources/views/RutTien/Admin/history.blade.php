                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        ...
                        <div class="kt-portlet__head-actions">
                            ...
                            {{--                                <a href="{{ route('formaddhoadon') }}{{isset($_GET['harvest_id']) ? '?harvest_id='.$_GET['harvest_id'] : ''}}"--}}
                            {{--                                   class="btn btn-brand btn-elevate btn-icon-sm">--}}
                            <a href="/admin/goi_dich_vu/view"
                               class="btn btn-brand btn-elevate btn-icon-sm">
...
//                        dd($listItem);
                    @endphp
                    @if(in_array('super_admin', $permissions)||(\Auth::guard('admin')->user()->roles()->orderBy('id', 'desc')->first()->id == 2))
                        <table class="table table-striped" border="1">
                            <thead class="kt-datatable__head">
                            <tr class="kt-datatable__row" style="left: 0px;">
                                <th style="display: none; text-align: center;"></th>
                                @php $count_sort = 0; @endphp
                                @foreach($module['list_admin'] as $field)
                                    @if($field['name'] == 'stk' || $field['name'] == 'chu_tk') @continue @endif
                                    <th data-field="{{ $field['name'] }}" style="text-align: center;"
                                        class="kt-datatable__cell kt-datatable__cell--sort {{ @$_GET['sorts'][$count_sort] != '' ? 'kt-datatable__cell--sorted' : '' }}"
                                        @if(isset($field['sort']))
                                            onclick="sort('{{ $field['name'] }}')"
                                            @endif
                                    @if($field['name'] == 'ngan_hang') colspan = '3' @endif
                                    >

                                        @if($field['name'] == 'ngan_hang') Thông tin người nhận
                                        @else {{ trans($field['label']) }} @endif
                                        @if(isset($field['sort']))
                            ...
                            @foreach($listItem as $item)
                                <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
                                    @if(!($field['name'] == 'stk' || $field['name'] == 'chu_tk'))
...
{{--            modal.style.display = "none";--}}
{{--            // Chuyển hướng sau khi xác nhận hủy--}}
{{--            window.location.href = btn.href;  // Lấy href từ thẻ a và chuyển hướng--}}
{{--        }--}}
