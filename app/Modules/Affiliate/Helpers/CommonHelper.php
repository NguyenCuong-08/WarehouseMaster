...
namespace App\Modules\Affiliate\Helpers;

use App\Models\Admin;
use App\Models\RoleAdmin;
use App\Models\Setting;
use App\Modules\Affiliate\Models\Bill;
use App\Modules\Affiliate\Models\TaiMuaHang;
use Auth;
...
class CommonHelper
{
    ...
    public static function checkAdmin($admin_id)
    {
        $admin = Admin::find($admin_id);
        if($admin->hasSuperAdminRole()) return true;
        elseif($admin->roles()->where('name', 'ke_toan')->exists()) return true;
        else return false;
    }
    public static function demSoGoiDauTuDaMua(){
        $number=0;
        $bills = Bill::all();
        foreach($bills  as  $bill){
            if(@$bill->goi_dich_vu->type==2){
                $number++;
            }
        }
        return $number;
    }
    public static function soTienHoaHongDaNhan($admin_id){
        $number=0;
        $number = LichSuGiaoDich::where('admin_id', $admin_id)
        ->where('type', 2)->sum('amount');

        return $number;
    }
    public static function soTienDaRut($admin_id){
        $number=0;
        $number = LichSuGiaoDich::where('admin_id', $admin_id)
            ->where('type', 1)->whereNull('sender_id')->sum('amount');

        return $number;
    }
    public static function demSoThanhVienGioiThieu($admin_id, $time = null){

        if ($time) {
            // Nếu $time được truyền vào, đếm số lượng thành viên tham gia trước hoặc vào thời gian đó
            $count = Admin::where('invite_by', $admin_id)
                ->where('ngay_tham_gia', '<=', $time)
                ->count();
        } else {
            // Nếu $time không được truyền vào, đếm số lượng thành viên có ngày mua đơn cuối không null
            $count = Admin::where('invite_by', $admin_id)
    ...
    public static function demSoTienHoanDaNhan($admin_id){
        //tính cả hoàn tiền con, hoàn tiền thu nhập trên thu nhập
        $total_money = LichSuGiaoDich::where('admin_id', $admin_id)->where('type', 2)->sum('amount');
        return $total_money;
    }

    public static function demSoDonDaMua($admin_id){
        $total_bill = Bill::where('admin_id', $admin_id)->count();
        return $total_bill;
    }
    public static function checkKhoaVi($admin_id){
        $total_money = self::demSoTienHoanDaNhan($admin_id);
        $total_bill = self::demSoDonDaMua($admin_id);
        $list_tai_mua = TaiMuaHang::orderBy('price', 'asc')->get();
        foreach ($list_tai_mua as $item){
            //nếu giá lơn hơn sẽ so sánh với số đơn hiện có, trả vè id của tai-mua-hnag nếu ko thỏa mãn
            if($total_money>=$item->price){
                if($total_bill>=$item->request_total_order){
                    continue;
                }
                return $item->id;
            }
    ...
    public static function checkKhoaVi60ngay(){
        //lấy ra ngày mua gần nhất
        $so_ngay_tai_mua = Setting::where('name', 'so_ngay_tai_mua')->first()->value;
        $ngay_mua_don_cuoi = Auth()->guard('admin')->user()->ngay_mua_don_cuoi;
        $now = Carbon::today();
        $so_ngay_chua_mua = $now->diffInDays($ngay_mua_don_cuoi);
        if($so_ngay_chua_mua>$so_ngay_tai_mua) return false;
        return true;
    }
    ...
    public static function has_permission($admin_id, $per_check = '')
    {
        try {
            ...
            dd($admin_id, $per_check, $ex->getMessage(), $ex->getLine(), $key_cache);
            return false;
        }
    }
