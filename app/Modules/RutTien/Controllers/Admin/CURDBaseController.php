...
use function App\Modules\GoiDichVu\Controllers\Admin\response;
use function App\Modules\GoiDichVu\Controllers\Admin\str_slug;
use function App\Modules\GoiDichVu\Controllers\Admin\trans;
...
//        dd(\Auth::guard('admin')->user()->roles()->orderBy('id', 'desc')->first()->id);
        //lọc phân quyền,nếu super_admin mới cho hiển thị full
        if(!\Auth::guard('admin')->user()->super_admin == 1 && !\Auth::guard('admin')->user()->roles()->orderBy('id', 'desc')->first()->id==2  ){
...
//                            dd($where);
    ...
    public function getDataAdd(Request $request) {
        $data['module'] = $this->module;
        $data['page_title'] = '' . trans($this->module['label']);
        $data['page_type'] = 'add';
        return $data;
    }
    ...
    public function getDataUpdate(Request $request, $item) {
        $data['module'] = $this->module;
        $data['result'] = $item;
        $data['page_title'] = 'Chỉnh sửa ' . __($this->module['label']);
