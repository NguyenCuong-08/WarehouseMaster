...
//use DB;
class ResultSearchProductController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'list' => [
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.product_name', 'label' => 'Tên sản phẩm'],
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.product_code', 'label' => 'Mã sản phẩm'],
//            ['name' => 'image', 'type' => 'image', 'label' => 'Ảnh'],
//            ['name' => 'price', 'type' => 'text', 'label' => 'Giá'],
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.day_hang', 'label' => 'Dãy hàng'],
//            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.ke_hang', 'label' => 'Kệ hàng'],
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.tang_ke', 'label' => 'Tầng hàng'],
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.o_so', 'label' => 'Ô hàng'],
            ['name' => 'action', 'type' => 'custom', 'td' => 'Product.search.td.o_hang', 'label' => 'Tên ô'],
            ['name' => 'quantity', 'type' => 'text', 'label' => 'Số lượng'],
            ['name' => 'date', 'type' => 'date_vi', 'label' => 'Ngày nhập'],
...
//                    'model' => Category::class, 'display_field' => 'name', 'group_class' => 'col-md-4'],
                // Dãy hàng
//                ['name' => 'day_hang_id', 'type' => 'select2_model', 'class' => '', 'label' => 'Dãy hàng',
//                    'model' => DayHang::class, 'display_field' => 'name', 'group_class' => 'col-md-12'],
//
//                // Kệ hàng
//                ['name' => 'ke_hang_id', 'type' => 'select2_model', 'class' => '', 'label' => 'Kệ hàng',
//                    'model' => KeHang::class, 'display_field' => 'name', 'group_class' => 'col-md-12'],
//
//                ['name' => 'tang_ke_id', 'type' => 'select2_model', 'class' => '', 'label' => 'Tầng kệ',
//                    'model' => TangKe::class, 'display_field' => 'name', 'group_class' => 'col-md-12'],  // Add this line
//                // o hang
//                ['name' => 'o_hang_id', 'type' => 'select2_model', 'class' => '', 'label' => 'Ô hàng',
//                    'model' => OHang::class, 'display_field' => 'name', 'group_class' => 'col-md-12'],

//                ['name' => 'tel', 'type' => 'custom', 'field' => 'Product.search.field.o_hang', 'class' => 'required', 'group_class' => 'col-md-3'],
                ['name' => 'quantity', 'type' => 'text', 'label' => 'Số lượng'],
...
//            $day_hang = DayHang::all();
//            return view('KhoHang.result-product', ['day_hang'=> $day_hang, 'product'=>$product])->with($data);
//        }else{
...
//    }
    ...
    public function checkProductPost(Request $request)
    {
        ...
        $product = Product::where('code', $request->code)->first();
        $oHang = OHang::where('name', $request->name)->first();
        if($oHang){
            if ($product) {
                // Lưu code và slot vào session
                session(['code' => $request->code, 'oHangName' => $request->name]);

                // Lấy dữ liệu khác nếu cần
                $data = $this->getDataList($request);
                CommonHelper::one_time_message('success', 'Check mã thành công');

                $oHang = OHang::where('name', $request->name)->first();

//            $o_hang_id=$oHang->id;
//            $product_id= $product->id;
//            $searchOHangProduct=OHangProduct::where('o_hang_id', $o_hang_id)
//            ->where('product_id', $product_id)->exists();
//             dd($searchOHangProduct);
//            if(!$searchOHangProduct){
//                dd("ok");
//                CommonHelper::one_time_message('error', 'Check mã thành công');
//                return redirect()->back();
//            }
                // Kiểm tra nếu tìm thấy oHang
                if ($oHang) {
                    // Lưu id của oHang vào session
                    session(['oHangId' => $oHang->id]);
                }
                $day_hang = DayHang::all();

                // Trả về view với dữ liệu đã lưu trong session
                return view('KhoHang.result-product', [
                    'day_hang' => $day_hang,
                    'product' => $product,
                    'code' => session('code'), // Truyền code từ session vào view
                    'aisle' => session('aisle'),
                    'shelf' => session('shelf'),
                    'level' => session('level'),
                    'slot' => session('slot'),  // Truyền slot từ session vào view
                    'oHangId' => session('oHangId'),

                ])->with($data);
            } else {
                CommonHelper::one_time_message('error', 'Không tìm thấy sản phẩm với mã đã nhập. Vui lòng nhập sản phẩm mới.');
                return redirect()->back(); // Trả về trang trước đó
            }
        }else{
            CommonHelper::one_time_message('error', 'Không tìm thấy Ô hàng. Vui lòng nhập ô hàng mới.');
            return redirect()->back(); // Trả về trang trước đó
        }
    ...
    public function showResultPage(Request $request)
    {
        ...
        if ($code && $slot) {
            // Lấy dữ liệu khác nếu cần
            $data = $this->getDataList($request);
            // Nếu có, bạn có thể thực hiện các thao tác cần thiết, như lấy thông tin sản phẩm hoặc hiển thị nó
        ...
        // Nếu không có dữ liệu, có thể trả về một view thông báo hoặc redirect
        return redirect()->route('check-product')->with('message', 'Không có dữ liệu sản phẩm trong session.');
    }
//    public function checkProductPost(Request $request, $id = null)
//    {
//        if ($request->isMethod('post')) {
//            // Tìm sản phẩm theo code từ request POST
//            $product = Product::where('code', $request->code)->first();
//            if ($product) {
//                $data = $this->getDataList($request);
//                CommonHelper::one_time_message('success', 'Check mã thành công, nhập thông tin nhập kho');
...
//                return view('KhoHang.result-product', ['day_hang' => $day_hang, 'product' => $product])->with($data);
//            } else {
//                CommonHelper::one_time_message('error', 'Không tìm thấy sản phẩm với mã đã nhập.');
//                return redirect()->back();
//            }
//        } elseif ($id) {
//            // Lấy lại sản phẩm theo ID trong request GET
...
//                return view('KhoHang.result-product', ['day_hang' => $day_hang, 'product' => $product])->with($data);
//            } else {
//                CommonHelper::one_time_message('error', 'Không tìm thấy sản phẩm.');
//                return redirect()->back();
//            }
//        } else {
//            return redirect()->back();
...
//            return redirect()->route('nhap-hang')->with('product', $product);
    ...
    public function getIndex(Request $request)
    {
        $data = $this->getDataList($request);
//        $this->billcheck();die();
        return view('KhoHang.result-product')->with($data);
    }
    ...
    public function suaHangPost(Request $request)
    {
        $product = Product::where('code', $request->code)->first();
        // Tìm OHangProduct với o_hang_id và product_id đã tồn tại
        $ohangProduct = OHangProduct::where('id', $request->code)
            ->first();

        if ($ohangProduct) {
            // Nếu bản ghi đã tồn tại, sửa quantity
        ...
        CommonHelper::one_time_message('success', 'Cập nhật thông tin thành công');
        return redirect()->route('check-product');
...
//'oHangId' => session('oHangId'),
    public function appendWhere($query, $request)
    {
        if (!empty($request->all())) {
            ...
            $product = Product::where('code', session('code'))->first();
            $oHang = OHang::where('name', session('oHangName'))->first();
        }
//        $product = Product::where('code', session('code'))->first();
//        $oHang = OHang::where('id', session('oHangId'))->first();
        $query->where('product_id', $product->id)->where('o_hang_id', $oHang->id);
        return $query;
    }
//    public function appendWhere($query, $request)
//    {
//        // Try to get the product by code from the database
//        $product = Product::where('code', $request->code)->first();
//
//        // Try to get the OHang by slot from the database
//        $oHang = OHang::where('id', $request->slot)->first();
//
//        // Check if product is found
//        if ($product) {
//            $productId = $product->id;
//        } else {
//            // If product is not found, check the session for the product code
//            $productId = session('productId'); // Ensure you have saved this in the session earlier
//        }
//
//        // Check if oHang is found
//        if ($oHang) {
//            $oHangId = $oHang->id;
//        } else {
//            // If oHang is not found, check the session for the oHangId
//            $oHangId = session('oHangId'); // Ensure you have saved this in the session earlier
//        }
//
//        // Apply the query conditions using the ids
//        $query->where('product_id', $productId)->where('o_hang_id', $oHangId);
//
//        return $query;
//    }
...
//                $day_hang = $item->oHang->tang_ke->ke_hang->day_hang->name;
//                $ke_hang = $item->oHang->tang_ke->ke_hang->name;
//                $tang_ke = $item->oHang->tang_ke->name;
//                $id_o_hang_product = $request->id;
//                $o_hang = $item->oHang->name;
//                $so_luong = $item->quantity;
//                $code = $item->product->code;
//                return view('Product.search.field.o_hang')->with($data)->with('day_hang', $day_hang)->with('o_hang', $o_hang)->with('so_luong', $so_luong)->with('code', $code)->with('id_o_hang_product', $id_o_hang_product)->with('ke_hang', $ke_hang)->with('tang_ke', $tang_ke);
//            } else if ($_POST) {
//                \DB::beginTransaction();
//
//                $validator = Validator::make($request->all(), [
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('Product.search.td.edit')->with($data);
            } else if ($_POST) {
                \DB::beginTransaction();
