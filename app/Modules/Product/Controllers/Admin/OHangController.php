...
//use DB;
class OHangController extends CURDBaseController
{

    protected $module = [
        'code' => 'o_hang',
        'table_name' => 'o_hang',
        'label' => 'Quản lý ô hàng',
        'modal' => '\App\Modules\Product\Models\OHang',
        'list' => [
            ['name' => 'name', 'type' => 'text', 'label' => 'Tên ô hàng'],
            ['name' => 'day_hang_id', 'type' => 'relation', 'object' => 'day_hang', 'display_field' => 'name', 'label' => 'Dãy hàng'],

            ['name' => 'tang_hang', 'type' => 'text', 'label' => 'Tầng'],
            ['name' => 'o_hang', 'type' => 'text', 'label' => 'Ô'],
            ['name' => 'des', 'type' => 'text', 'label' => 'Miêu tả'],
        ...
        'form' => [
            'general_tab' => [
                ...
                ['name' => 'day_hang_id', 'type' => 'select2_model', 'class' => '',  'label' => 'Dãy hàng',
                    'model' => DayHang::class, 'display_field' => 'name', 'group_class' => 'col-md-8', 'class' => 'required'],
                ['name' => 'tang_hang', 'class' => 'required', 'type' => 'select2', 'options' => [
                    '' => '',
                    '1' => 'Tầng 1',
                    '2' => 'Tầng 2',
                    '3' => 'Tầng 3',
                ...
                ], 'label' => 'Tầng', 'group_class' => 'col-md-8'],
                ['name' => 'o_hang','class' => 'required',  'type' => 'select2', 'options' => [
                    '' => '',
                    '1' => 'Ô 1',
                    '2' => 'Ô 2',
                    '3' => 'Ô 3',
                ...
                ], 'label' => 'Ô', 'group_class' => 'col-md-8'],
                ['name' => 'name', 'type' => 'text', 'class' => 'required', 'label' => 'Tên ô hàng', 'group_class' => 'col-md-12'],
                
                ['name' => 'des', 'type' => 'textarea', 'class' => 'required', 'label' => 'Mô tả', 'group_class' => 'col-md-12'],
            ],
            'more_info_tab' => [
            ],
...
//        dd('khôi');
        ...
        try {
            if (!$_POST) {
                ...
                return view('Product.ohang.add')->with($data);
            } else if ($_POST) {
//                \DB::beginTransaction();

                $validator = Validator::make($request->all(), [
                    'name' => 'required|unique:o_hang,name',
                ], [
                    'name.unique' => 'Ô này đã tồn tại',
                ]);
                if ($validator->fails()) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('Product.ohang.edit')->with($data);
            } else if ($_POST) {
                \DB::beginTransaction();

                $validator = Validator::make($request->all(), [
                    'name' => 'required|unique:o_hang,name,' . $item->id
                ], [
        ...
        if ($file) {
            ...
            if (!empty($data) && $data->count()) {
                foreach ($data as $key => $value) {
                    ...
                    if (!empty($value->ten) && !empty($value->day)) {
//                        dd(1);
                        $existingProduct = OHang::where('name', $value->ten)->first();
                        $tnt = DayHang::where('name', $value->day)->first();
                        if ($tnt) { // Kiểm tra $tnt có tồn tại hay không
                            $id = $tnt->id;
                        } else {
                        ...
                        if (!$existingProduct) {
                            ...
                            $product->des = $value->mo_ta;
                            $product->day_hang_id = $id; // Cần xử lý nếu bạn cần upload ảnh
                            $product->tang_hang = $value->tang_hang;
                            $product->o_hang = $value->o_hang;
