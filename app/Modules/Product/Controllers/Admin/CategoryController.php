...
//use DB;
class CategoryController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'label' => 'Nhóm sản phẩm',
        'modal' => '\App\Modules\Product\Models\Category',
        'list' => [
            ['name' => 'name', 'type' => 'text', 'label' => 'Tên nhóm'],
            ['name' => 'des', 'type' => 'text', 'label' => 'Mô tả nhóm',],
            ['name' => 'action', 'type' => 'custom', 'td'=>'Product.list.td.action', 'label' => 'Hành động',],
        ],
        'form' => [
            'general_tab' => [
                ['name' => 'name', 'type' => 'text', 'class' => 'required', 'label' => 'Tên nhóm', 'group_class' => 'col-md-12'],
                ['name' => 'des', 'type' => 'textarea', 'class' => 'required', 'label' => 'Mô tả nhóm', 'group_class' => 'col-md-12'],
...
//                    dd($value);
                    ...
                    if (!empty($value->ten_nhom_san_pham)) {
                        $existingProduct = Category::where('name', $value->ten_nhom_san_pham)
                            ->first();

                        // Nếu sản phẩm chưa tồn tại, thì tạo mới
                        if (!$existingProduct) {
                            $product = new Category();
                            $product->name = $value->ten_nhom_san_pham;
                            $product->des = $value->mo_ta_nhom;
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                $validator = Validator::make($request->all(), [
                    'name' => 'required|unique:categories,name,'. $item->id,
                ], [
                    'name.unique' => 'Nhóm này tồn tại',
                ]);
