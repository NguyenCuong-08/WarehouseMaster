...
class CURDBaseController extends Controller
{
    ...
    //khôi
    public function getDataListChild(Request $request) {
        //  Filter
        $where = $this->filterSimple($request);
        $listItem = $this->model->whereRaw($where);
        $listItem = $this->quickSearch($listItem, $request);
        ...
        $listItem = $this->appendWhereChild($listItem, $request);
    ...
    public function getDataListSearch(Request $request) {
        //  Filter
        $where = $this->filterSimple($request);
        $listItem = $this->model->whereRaw($where);
        $listItem = $this->quickSearch($listItem, $request);
        ...
        $listItem = $this->appendWhereSearch($listItem, $request);
    ...
    public function appendWhereSearch($query, $request)
    {
//        dd($request->all());
        $admin_search = Admin::where('tel', $request->sdt)->first();
//        dd($admin_search);
//        dd($admin_search);
        if($admin_search){
            $query = $query->where('invite_by', $admin_search->id);

        }
        else{
//            dd('vào đy');
            //miễn sao nó ko ra gì trong trường hơ này
            $query = $query->where('invite_by', 'not_exist')->where('tel', $request->sdt);
//            dd($query);
        }
        return $query;
    }
        ...
        if (@$r->quick_search != '') {
            $listItem = $listItem->where(function ($query) use ($r) {
                foreach (explode(',', $this->quick_search['fields']) as $field) {
                    if(trim($field)=='id'){
                        $query->orWhere('id',$r->quick_search);//  truy vấn các tin thuộc các danh mục con của danh mục hiện tại
