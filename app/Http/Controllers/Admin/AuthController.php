...
//hàm khóa tài khoản khi đăng nhập sai mật khẩu quá 5 lần
    ...
    {
        $settings = Setting::select(['name', 'value'])->where('type', 'role_tab')
                                                      ->pluck('value', 'name')
                                                      ->toArray();
...
//            'tel.unique' => 'Số điện thoại đã tồn tại!',
    ...
    public function getEmailForgotPassword(Request $request)
    {
        if (!$_POST) {
            ...
            $admin = $query->first();
            //random password base 64
            $randomString = Str::random(10);
            $admin->change_password = base64_encode($randomString);
            $admin->password = bcrypt($admin->change_password);
            $admin->save();
            $mailSetting = Setting::whereIn('type', ['mail'])->pluck('value', 'name')->toArray();

            if (empty($mailSetting)) {
                CommonHelper::one_time_message('error', 'Xin vui lòng cấu hình mail server!');
                return back();
            } else {
                $data = [
                    'email' => $admin->email,
                    'subject' => env('DOMAIN') . ' - Khôi phục mật khẩu',
                    'content' => 'Xin chào ' . $admin->name . ',<br>
                                  ' . 'Bạn đã yêu cầu lấy lại mật khẩu tại ' . env('DOMAIN') . '<br>
                                  ' . 'Thông tin tài khoản của bạn là:<br>
                                    ' . 'Email: <b>' . $admin->email . '</b><br>
                                  ' . 'Mật khẩu mới của bạn là: <b>' . $admin->change_password . '</b><br>
                                  ' . ' Vui lòng đăng nhập và đổi mật khẩu để bảo mật tài khoản của bạn! <a href="https://crm.anicca.edu.vn/admin/login">Tại đây</a> <br>
                                  ' . 'Trân trọng,<br>
                                    ' . 'Ban quản trị ' . env('DOMAIN') . '<br>'
                                   ];
